...
function normalize(toc, rel, comparer, model) {
  if (!toc) return;
  toc.level = 1;
  if (toc.items && toc.items.length > 0) {
    toc.leaf = false;
    for (var i = toc.items.length - 1; i >= 0; i--) {
      normalizeCore(toc.items[i], rel, 2, comparer, model);
    };
  } else {
    toc.items = [];
    toc.leaf = true;
...
function normalizeCore(item, rel, level, comparer, model) {
  item.pathActive = false;
  item.active = false;
  item.level = level;
  if (comparer && comparer(item)) {
    item.active = true;
    item.pathActive = true;
    if (model) {
      model._pathActive = true;
    }
  }
